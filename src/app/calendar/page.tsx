  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-neutral-200 bg-white p-4">
        <div className="mb-4 text-lg font-semibold">{monthLabel}</div>
        ...
        <div className="mt-2 grid grid-cols-7 gap-2 text-center">
          {cells.map((day, index) => (
            <div
              key={`${day ?? "empty"}-${index}`}
              className={`h-16 rounded-md border border-neutral-200 text-sm flex items-center justify-center ${
                day ? "bg-neutral-50" : "bg-transparent border-transparent"
              }`}
            >
              {day ?? ""}
            </div>
